...
// NoteApp.UI/Assets/Editor/editor-entry.js
var markdownLoadPromise = null;
function loadMarkdownExtension() {
  if (!markdownLoadPromise) {
    markdownLoadPromise = import("./chunks/dist-57J3CMPD.js").then((module) => module.markdown()).then((extension) => {
...
function createEditor(hostElement, onChange) {
  ...
  function findNext2(query, matchCase) {
    const text = getText();
  ...
  function findPrevious2(query, matchCase) {
    const text = getText();
    const needle = matchCase ? query : query.toLowerCase();
    const hay = matchCase ? text : text.toLowerCase();
  ...
  function replaceNext2(query, replacement, matchCase) {
    const selected = getSelection();
    const selectedText = selected.text;
    const matches = matchCase ? selectedText === query : selectedText.toLowerCase() === query.toLowerCase();
    ...
    const index = findNext2(query, matchCase);
    if (index >= 0) {
  ...
  function replaceAll2(query, replacement, matchCase) {
    const text = getText();
    const needle = matchCase ? query : query.toLowerCase();
    const hay = matchCase ? text : text.toLowerCase();
    let index = 0;
